...
class _ResidualBlock(nn.Module):
    def __init__(
        ...
        # Base parameters controlling temporal receptive field and block position
        ...
        # Input dimension:
        #   - First block sees original feature dimension (input_size)
        #   - Subsequent blocks see num_filters from previous blocks
        input_dim = input_size if nr_blocks_below == 0 else num_filters
        # Output dimension:
        #   - Last block projects to target_size
        #   - Intermediate blocks stay in num_filters space
        output_dim = (
            target_size if nr_blocks_below == num_layers - 1 else num_filters
        )

        # First causal/dilated conv:
        #   receptive field grows with dilation_base ** nr_blocks_below
        ...
        # Second conv maps from num_filters to either:
        #   - num_filters (internal blocks)
        #   - target_size (last block)
        ...
        # If residual branch has different channels, define 1x1 projection
        # to make x and residual compatible for the final addition.
    ...
    def forward(self, x):
        ...
        # ---------------------------------------------------------------------
        # First step (conv1 + ReLU + dropout)
        # ---------------------------------------------------------------------
        # Compute left padding to ensure causal convolution:
        #   padding = dilation * (kernel_size - 1)
        # so that conv only sees past time steps.
        left_padding = (self.dilation_base**self.nr_blocks_below) * (
            self.kernel_size - 1
        )
        # Pad only on the left (past), keep future unpadded for causality.
        x = F.pad(x, (left_padding, 0))
        # First dilated conv + non-linearity + MC dropout
        x = self.dropout1(F.relu(self.conv1(x)))

        # ---------------------------------------------------------------------
        # Second step (conv2 + optional ReLU + dropout)
        # ---------------------------------------------------------------------
        # Same causal padding before second conv        
        x = F.pad(x, (left_padding, 0))
        x = self.conv2(x)

        # Only apply ReLU if this is not the last block:
        # last block can output linear activations for final mapping.
        if self.nr_blocks_below < self.num_layers - 1:
            x = F.relu(x)

        # Second MC dropout for regularization / uncertainty estimation
        x = self.dropout2(x)

        # ---------------------------------------------------------------------
        # Residual connection
        # ---------------------------------------------------------------------
        # If channels differ (e.g., last block mapping to target_size),
        # project residual via 1x1 conv so it can be added to x.
...
class TCNModel(PastCovariatesTorchModel):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            **self._extract_torch_model_params(**self.model_params)
        )
