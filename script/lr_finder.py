...
from src.model import LightningWrapper, ModelType, model_factory, init_weights, compute_mav
...
class LRFinder:
        ...
        for i, lr in enumerate(lr_schedule):
            ...
            try:
                ...
                if len(batch) == 3:  # (x, y, weights)
                    x, y, weights = batch
                    x, y, weights = x.to(self.model.device), y.to(self.model.device), weights.to(self.model.device)
                ...
                if i == 0:
                    ...
                    smoothed_loss = smooth_f * loss_val + (1 - smooth_f) * self.loss_history[-2]
                ...
                if i % 10 == 0:
                    print(f"Iteration {i}/{num_iter}, LR: {lr:.2e}, Loss: {loss_val:.4f}")
    ...
    def plot(self, skip_start: int = 10, skip_end: int = 5, log_lr: bool = True, 
             save_path: Optional[Path] = None, show_plot: bool = True):
        """Plot the learning rate finder results"""
        if len(self.lr_history) == 0:
            raise ValueError("No data to plot. Run range_test first.")
            
        # Skip some points at start and end
        lrs = self.lr_history[skip_start:-skip_end] if skip_end > 0 else self.lr_history[skip_start:]
        losses = self.loss_history[skip_start:-skip_end] if skip_end > 0 else self.loss_history[skip_start:]
...
def load_data_for_cluster(
    ...
    train_file = dataloader_dir / f"train_loader_{store_cluster}_{item_cluster}.pt"
    val_file = dataloader_dir / f"val_loader_{store_cluster}_{item_cluster}.pt"
...
def run_lr_finder(
    ...
    logger.info(f"Running LR Finder for store_cluster={store_cluster}, item_cluster={item_cluster}")
    logger.info(f"Model type: {model_type.value}")
    
    # Load data
    train_loader, val_loader = load_data_for_cluster(dataloader_dir, store_cluster, item_cluster)
    ...
    plot_path = output_dir / f"lr_finder_{store_cluster}_{item_cluster}_{model_type.value}.png"
    ...
    csv_path = output_dir / f"lr_finder_{store_cluster}_{item_cluster}_{model_type.value}.csv"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Learning Rate Finder for Favorita Sales Forecasting")
    
    parser.add_argument("--dataloader_dir", type=str, required=True,
                       help="Directory containing the dataloaders")
    parser.add_argument("--store_cluster", type=int, required=True,
                       help="Store cluster ID")
    parser.add_argument("--item_cluster", type=int, required=True,
                       help="Item cluster ID")
    parser.add_argument("--model_type", type=str, default="ShallowNN",
                       choices=["ShallowNN", "TwoLayerNN", "ResidualMLP"],
                       help="Model type to use")
    parser.add_argument("--output_dir", type=str, default="./output/lr_finder",
                       help="Output directory for results")
    parser.add_argument("--start_lr", type=float, default=1e-7,
                       help="Starting learning rate")
    parser.add_argument("--end_lr", type=float, default=1.0,
