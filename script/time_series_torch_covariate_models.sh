...
if [[ -z "$METRICS_FN" ]]; then
  METRICS_FN="${METRICS_DIR}/${DATE}_2013_2014_store_2000_item_cyc_features_TFT_past_future_covs_metrics.csv"
fi
...
LOG_FILE="${LOG_DIR}/${TIMESTAMP}_time_series_torch_covariate_models.log"
...
python "${SCRIPT_DIR}/time_series_covariates.py" \
  --data_fn "$DATA_FN" \
